...
SELECT 
    ...
    CONCAT_WS(' ', m.firstname, m.surname) AS member_name,
    CASE
        WHEN b.memid = 0 THEN f.guestcost * 2
        ELSE f.membercost * 2
    END AS cost
FROM 
    Bookings b
...
ORDER BY 
    m2.surname, m2.firstname, m1.surname, m1.firstname;
...
SELECT
    f.name AS facility_name,
    DATE_FORMAT(b.starttime, '%Y-%m') AS month_year,  -- Format date as YYYY-MM
    COUNT(b.bookid) AS usage_count
FROM
    Bookings b
JOIN
